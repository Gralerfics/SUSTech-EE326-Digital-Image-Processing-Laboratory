...
def compute_spectrum(f): # Shifted
...
def compute_FH_in_pair(f, h, pad_mode='min'): # Shifted
    ...
    return (compute_spectrum(f_p), 1j * np.imag(compute_spectrum(h_p)) * shifter)


def recover_spatial(G, regulator=NonRegulator):
