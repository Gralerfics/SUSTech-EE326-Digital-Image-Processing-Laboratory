...
img = persistence.load_gray('res/Q5_3.tif')
...
persistence.show(H_p)
persistence.show(frequency.magnitude_spectrum_disp(frequency.compute_spectrum(img_p) * H_p))

img = frequency.filter_freq_pad(img, H_p, is_H_padded=True, regulator=regulator.GrayCuttingRegulator)
persistence.show(img)
